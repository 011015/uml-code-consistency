...
export function getGenTemplate() {
  return $(
    ...
    $(go.Shape, { toArrow: "Triangle", fill: "white", scale: 3 })
  );
}
...
export function getRealTemplate() {
  return $(
    ...
    linkStyle(),
    new go.Binding("points").makeTwoWay(),
    $(go.Shape, { isPanelMain: true, strokeDashArray: [3, 2], strokeWidth: 3 }),
    $(go.Shape, {
      isPanelMain: true,
      stroke: "transparent",
      strokeWidth: 30,
...
export function getDepTemplate() {
  return $(
    ...
    $(go.Shape, { toArrow: "OpenTriangle", scale: 3 })
  );
}
...
export function getCompTemplate() {
  return $(
    ...
    $(go.Shape, { toArrow: "StretchedDiamond", scale: 3 })
  );
}
...
export function getAggTemplate() {
  return $(
    ...
    $(go.Shape, { toArrow: "StretchedDiamond", fill: "white", scale: 3 })
  );
}
...
function makePort(
  ...
  return $(
    ...
    new go.Binding("desiredSize", "btnDesiredSize", go.Size.parse).makeTwoWay(
      go.Size.stringify
    )
  );
}
    ...
    $("ContextMenuButton", $(go.TextBlock, "Add Property"), {
      click: (e, obj: any) => {
        ...
        if (node.properties && node.properties.length) {
          ...
          const titleFont = (node.titleFont as string).split(" ");
          const px = titleFont[1];
          const fontSize = Math.floor(
            (parseInt(px.slice(0, px.length - 2)) / 0.75) * 0.65
          );
          font = fontSize + "px " + titleFont[2];
          node.properties = [
            {
              key: uuidv4(),
              name: "New Property",
  ...
  return $(
    ...
    $(
      ...
      $(
        ...
        "Properties",
        { row: 1 },
        new go.Binding("font", "contentFont"),
